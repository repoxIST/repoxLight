...
import pt.utl.ist.marc.iso2709.BatchInfo.Sgb;
...
public class Iso2709TypeDetector {
    ...
    private static final Logger log = Logger
            .getLogger(Iso2709TypeDetector.class);
    ...
    public static BatchInfo.CharSet detectCharSet(File isoFile) throws IOException {
        ...
        for (; nextC != -1 && cnt < 20000; cnt++) {
            ...
            if (c == 0x82 || c == 0x87 || c == 0x84 || c == 0xA1 || c == 0xA2 || c == 0x88 || c == 0x94 || c == 0xA3 || c == 0xC6 || c == 0xA0) {
                pb4Clues++;
            } else if (((c == 0xC2 || c == 0xC3 || c == 0xC4) && (nextC == 0x62 || nextC == 0x65 || nextC == 0x69 || nextC == 0x6F || nextC == 0x75)) ||
                    (c == 0xD0 && (nextC == 0x63 || nextC == 0x43))) {
                horizonClues++;
            } else if (((c == 0xE2 || c == 0xE1 || c == 0xE4) && (nextC == 'a' || nextC == 'e' || nextC == 'i' || nextC == 'o' || nextC == 'u' || nextC == 'A' || nextC == 'E' || nextC == 'I' || nextC == 'O' || nextC == 'U')) ||
                    (c == 0xF0 && (nextC == 'c' || nextC == 'C'))) {
                anselClues++;
        ...
        if (cnt > 500) {//if it read at least this number of chars
            if (pb4Clues > horizonClues && pb4Clues > 2 && iso8859Clues < pb4Clues * 2)
                return CharSet.CP850;
            if (horizonClues > 2 && iso8859Clues < horizonClues * 2)
                return CharSet.HORIZON;
            if (anselClues > 2 && iso8859Clues < anselClues * 2)
                return CharSet.ANSEL;
            return CharSet.ISO8859_1;
        } else {
            if (pb4Clues > horizonClues && pb4Clues > 1)
                return CharSet.CP850;
            if (horizonClues > 1)
                return CharSet.HORIZON;
            return CharSet.ISO8859_1;
