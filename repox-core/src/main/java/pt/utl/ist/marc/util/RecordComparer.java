...
public class RecordComparer {
    ...
    public enum RecordEngulfing {None, AcontainsB, BcontainsA, Equal}
    ...
    public boolean areEqual(Record recA, Record recB) {
        ...
        if (!fieldsToIgnore.contains(0)) {
            if (!leadersAreEquals(recA.getLeader(), recB.getLeader())
                    ) {
                return false;
            }
        }
        ...
        while (!finished) {
            ...
            if (tagB != tagA) {
                ...
                if (fldA.isControlField()) {
                    if (!equals(fldA.getValue(), fldB.getValue()))
                        return false;
                } else {
                    if (fldA.getInd1() != fldB.getInd1() || fldA.getInd2() != fldB.getInd2())
                        return false;
    ...
    public Tuple<Integer, RecordEngulfing> compareRecords(Record recA, Record recB) {
        ...
        if (engulf == RecordEngulfing.Equal)
            return new Tuple<Integer, RecordEngulfing>(100, engulf);
    ...
    protected RecordEngulfing compareFields(Field fA, Field fB) {
        if (fA.isControlField()) {
            return equals(fA.getValue(), fB.getValue()) ? RecordEngulfing.Equal : RecordEngulfing.None;
        }
        ...
        if (engulfAtoB == RecordEngulfing.Equal && engulfBtoA == RecordEngulfing.Equal) {
            return RecordEngulfing.Equal;
        }
        if (engulfAtoB == RecordEngulfing.AcontainsB) {
            return RecordEngulfing.AcontainsB;
        }
        if (engulfBtoA == RecordEngulfing.BcontainsA) {
            return RecordEngulfing.BcontainsA;
        }
    ...
    public static boolean leadersAreEquals(String leadA, String leadB) {
        return equals(leadA.substring(5, 10), leadB.substring(5, 10), false) ||
                !equals(leadA.substring(17, 20), leadB.substring(17, 20), false);
    }
}
