...
public class DataSourcesMonitor {
    ...
    private DataSourcesMonitor() {
        configurationFile = new File(ConfigSingleton.getRepoxContextUtil().getRepoxManager().getConfiguration().getXmlConfigPath(),
                RepoxContextUtilDefault.DATA_SOURCES_STATE_FILENAME);
    }
    ...
    private void loadXML() throws DocumentException, ParseException {
        ...
        if (!document.getRootElement().elements("dataSourceState").isEmpty()) {
            ...
            for (Element currentDataSourceState : (List<Element>) document.getRootElement().elements("dataSourceState")) {
                dataSourcesState.put(currentDataSourceState.elementText("dataSourceId"),
                        Boolean.valueOf(currentDataSourceState.elementText("state")));
            }
        }
    }
    ...
    /**
     * True if calendar has 0 hours and there was no monitoring in the last 2 hours.
     *
     * @param calendar
     */
    public boolean isTimeForMonitoring(Calendar calendar) {
        if (calendar.get(Calendar.HOUR_OF_DAY) == 0
                && (lastMonitoringTime == null || calendar.getTimeInMillis() - lastMonitoringTime.getTimeInMillis() > MIN_WAIT_TIME_MONITOR)) {
            return true;
        }
    ...
    /**
     * Receives a list of Data Sources and monitors the state of the Data Sources, saving it in the property dataSourcesState.
     *
