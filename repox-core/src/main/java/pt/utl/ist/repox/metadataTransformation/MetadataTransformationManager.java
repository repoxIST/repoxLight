...
//	}
    ...
    public synchronized void saveMetadataTransformation(MetadataTransformation metadataTransformation,
                                                        String oldMtdTransId) throws AlreadyExistsException, IOException,
            DocumentException, SameStylesheetTransformationException {
        MetadataTransformation savedMtdTrans;
        boolean isNewTransformation = oldMtdTransId.isEmpty();

        if (isNewTransformation)
            checkTransformationExists(metadataTransformation.getId(), metadataTransformation.getSourceFormat(),
                    metadataTransformation.getDestinationFormat(), metadataTransformation.getStylesheet());
            ...
            if (metadataTransformation.isDeleteOldFiles()) {
                ...
                if (!oldFilename.equals(FilenameUtils.removeExtension(metadataTransformation.getStylesheet())))
                    TransformationsFileManager.deleteXslFile(oldFilename, xsltDir);
                //delete xmap?
                if (old.isEditable())
                    TransformationsFileManager.deleteXmapFile(oldFilename, xmapDir);
            }
...
//		metadataTransformations = loadMetadataTransformations();
    ...
    public synchronized void checkTransformationValidity(String transformId, String xslFileName, String oldTransfId)
            throws AlreadyExistsException, SameStylesheetTransformationException, DocumentException, IOException { //todo tests?
        ...
        if (isNewTransformation)
            for (String sourceFormat : metadataTransformations.keySet()) {
                ...
                for (MetadataTransformation metadataTransformation : transformationsList) {
                    ...
                    if (bIdAlreadyExists)
                        throw new AlreadyExistsException(transformId);

                    Boolean bFilenameAlreadyExists = metadataTransformation.getStylesheet().toLowerCase().equals(xslFileName.toLowerCase());
                    if (bFilenameAlreadyExists)
                        throw new SameStylesheetTransformationException(xslFileName);
                }
            }
            ...
            for (String sourceFormat : metadataTransformations.keySet()) {
                ...
                for (MetadataTransformation metadataTransformation : transformationsList) {
                    ...
                    if (bIdAlreadyExists && !bNewIdEqualsOld)
                        throw new AlreadyExistsException(transformId);
                    ...
                    if (bFilenameAlreadyExists && !bNewFilenameEqualsOld)
                        throw new SameStylesheetTransformationException(xslFileName);
                }
            }
        }
    ...
    private boolean checkTransformationExists(String transformId, String srcFormat, String destFormat, String xslFileName)
            throws AlreadyExistsException, SameStylesheetTransformationException {
        for (String sourceFormat : metadataTransformations.keySet()) {
            List<MetadataTransformation> transformationsList = metadataTransformations.get(sourceFormat);

            for (MetadataTransformation metadataTransformation : transformationsList) {
                if (metadataTransformation.getId().toLowerCase().equals(transformId.toLowerCase()))
                    throw new AlreadyExistsException(transformId);
...
//                    throw new SameSrcAndDestFormatTransformationException(srcFormat + "->" + destFormat);
                if (metadataTransformation.getStylesheet().toLowerCase().equals(xslFileName.toLowerCase()))
                    throw new SameStylesheetTransformationException(xslFileName);
            }
        }
        return false;
        ...
        for (String sourceFormat : metadataTransformations.keySet()) {
            ...
            while (transformationIterator.hasNext()) {
                ...
                if (metadataTransformation.getId().equals(metadataTransformationId)) {
                    ...
                    //Delete XMAP file
                    if (metadataTransformation.isEditable())
                        TransformationsFileManager.deleteXmapFile(fileName, xmapDir);
    ...
    protected synchronized void loadMetadataTransformations() {
        ...
        try {
            ...
            for (Element currentElement : metadataTransformationElements) {
                ...
                System.out.println("SERVER - MDRCOMPLIANT (STRING): "+currentElement.elementText("bMDRCompliant"));
                ...
                MetadataTransformation metadataTransformation = new MetadataTransformation(id, description, sourceFormat,
                        destinationFormat, stylesheet, editable, version2, destSchema, destNamespace);
