...
/**
 * Used to identify the following entities in REPOX: DataSource, Record, Timestamp
 * URNs in REPOX have the following structure:
 * <RepositoryURNPrefix>:<DataSourceId>:<RecordId>;<Timestamp>
 * <p/>
 * Examples:
 * urn:bn:repox:PORBASE
 * urn:bn:repox:PORBASE:1203923
...
public class Urn {
    ...
    public Urn(String urn) throws InvalidInputException {
        ...
        try {
            if (!urn.startsWith(repoxManager.getConfiguration().getBaseUrn()))
                throw new InvalidInputException(urn);
    ...
    public Object getRecordId() throws DocumentException, IOException {
        ...
        if (recordId.getClass().equals(idClass)) {
            return recordId;
        }
