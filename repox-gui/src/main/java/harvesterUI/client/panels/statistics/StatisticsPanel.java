...
public class StatisticsPanel extends ContentPanel {
    ...
    public StatisticsPanel() {
        ...
        if (HarvesterUI.getProjectType() == ProjectType.EUDML) {
            ...
            ToggleButton total = new ToggleButton("Total", new SelectionListener<ButtonEvent>() {
                @Override
                public void componentSelected(ButtonEvent ce) {
                    getStatistics(StatisticsType.ALL);
                }
            });
            total.setToggleGroup("stat_bar");
            topToolbar.add(total);
            ToggleButton internal = new ToggleButton("Internal", new SelectionListener<ButtonEvent>() {
                @Override
                public void componentSelected(ButtonEvent ce) {
                    getStatistics(StatisticsType.INTERNAL);
                }
            });
            internal.setToggleGroup("stat_bar");
            topToolbar.add(internal);
            ToggleButton external = new ToggleButton("External", new SelectionListener<ButtonEvent>() {
                @Override
                public void componentSelected(ButtonEvent ce) {
                    getStatistics(StatisticsType.EXTERNAL);
                }
            });
            external.setToggleGroup("stat_bar");
            topToolbar.add(external);
            ToggleButton yadda = new ToggleButton("Yadda", new SelectionListener<ButtonEvent>() {
                @Override
                public void componentSelected(ButtonEvent ce) {
                    getStatistics(StatisticsType.YADDA);
                }
            });
            yadda.setToggleGroup("stat_bar");
            topToolbar.add(yadda);
    ...
    private void parseStatistics() {
        ...
        if (HarvesterUI.getProjectType() == ProjectType.EUDML) {
            RepoxEudmlStatisticsUI repoxEudmlStatisticsUI = (RepoxEudmlStatisticsUI) statistics;
            type.add(createNewNode("Yadda", formatNumber(repoxEudmlStatisticsUI.getDataSourcesYadda())));
        }
