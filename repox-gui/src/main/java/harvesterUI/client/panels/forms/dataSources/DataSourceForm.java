...
//            dataSourceUI.setExportDirectory(exprtP != null ? exprtP.trim() : "");
        ...
        if (HarvesterUI.getProjectType() == ProjectType.EUDML && storeInYadda != null)
            dataSourceUI.setStoreInYadda(storeInYadda.getValue());
    ...
    // Eudml Only field
    protected void addYaddaCheckBox(FieldSet fieldSet) {
        if (HarvesterUI.getProjectType() == ProjectType.EUDML) {
            CheckBoxGroup storeInYaddaCBGroup = new CheckBoxGroup();
            storeInYadda = new CheckBox();
            storeInYaddaCBGroup.setFieldLabel(HarvesterUI.CONSTANTS.storeInYadda() + "?" + HarvesterUI.REQUIRED_STR);
            storeInYadda.setValue(true);
            storeInYaddaCBGroup.add(storeInYadda);
            fieldSet.add(storeInYaddaCBGroup);
        }
    }

    protected void editYaddaCheckBox(Boolean value) {
        if (HarvesterUI.getProjectType() == ProjectType.EUDML) {
            storeInYadda.setValue(value);
        }
    }
