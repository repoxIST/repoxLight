 ...
 */
public class DefaultFormLayout extends AnchorLayout {
    ...
    public DefaultFormLayout(int labelWidth) {
     ...
     */
    public DefaultFormLayout(FormPanel.LabelAlign labelAlign) {
        this();
    ...
    protected int adjustHeightAnchor(int height, Component comp) {
        if (comp instanceof Field<?>) {
            Field<?> f = (Field<?>) comp;
            if (!f.isHideLabel() && labelAlign.equals(FormPanel.LabelAlign.TOP)) {
    ...
    protected int adjustWidthAnchor(int width, Component comp) {
        if (comp instanceof Field<?>) {
            ...
            if (((GXT.isIE && !GXT.isStrict) || GXT.isIE6) && !labelAlign.equals(FormPanel.LabelAlign.TOP) && !f.isHideLabel()) {
                width -= 3;
            }
        }
    ...
    protected void onLayout(Container<?> container, El target) {
        ...
        if (fieldTemplate == null) {
            ...
            sb.append("<div role='presentation' class='x-form-element x-form-el-{0} defaultFormItemLabel' id='x-form-el-{0}' style='{3}'>");
